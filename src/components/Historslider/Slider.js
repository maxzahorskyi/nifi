...
class Slider extends Component {
  ...
  mouseMove = (e) => {
    if (this.state.dragging) {
      let selection = [...this.props.selection];
      selection[this.state.dragIndex] = this.props.scale.invert(e.nativeEvent.offsetX);
      this.props.onChange(selection);
    }
  };
        ...
        {selection.map((m, i) => {
          return (
            <g
              ...
              style={{
                outline: 'none',
                display: i === 1 ? 'inline' : 'none',
              }}>
              <circle style={handleStyle} r={10} cx={0} cy={12.5} fill="#ddd" strokeWidth="1" />
              <circle
                style={handleStyle}
